...
def _parse_args():
    ...
    parser.add_argument(
        ...
        help="save reports with desired statuses only "
        "(by default rejected reports are ignored)",
    )
    parser.add_argument(
        '-g', '--group', dest='group_filter',
        action='store', nargs='+', default=None,
        help="save reports from desired student groups only "
...
def get_user_choice(text=None, allowed_choices=None):
    ...
    if allowed_choices:
        while True:
            ...
            print(
                f"Please, choose one of the following values: "
                f"{allowed_choices} or press Ctrl+C to exit"
            )
...
def process_reports(
    ...
    # get reports
    res = sess.post(
        f"https://pro.guap.ru/gettask/{task['id']}",
        json={"task_id": task['id']}
    )
    # remove all non-alphanumeric characters from task name,
    # see https://stackoverflow.com/a/13593932
    task_name = re.sub(r'[^\w\-_\. ]', '_', task['name'])
    # remove trailing whitespaces and truncate to 255 characters
    # in order to support Windows
    task_name = task_name.strip()[:255]
    logger.info(
        "Reports for task '%s' will be saved to '%s'", task_name, output_dir
    )
    ...
    for report in res.json()['reports']['reports']:
        ...
        if not report['filelink']:
            logger.debug(
                "Report from '%s' (%s) has no file link and will be skipped "
                "(nothing to download)",
                report['user_fio'], report['group_num']
            )
            continue
        # skip reports that were already downloaded
        if report['filelink'] in downloaded_reports:
            logger.debug(
                "Report '%s' already downloaded to '%s' and will be skipped",
                report['filelink'], downloaded_reports[report['filelink']]
            )
            continue
        # skip reports from unwanted groups
        if group_filter and report['group_num'] not in group_filter:
            logger.debug(
                "Report from group '%s' (%s) is ignored by filter",
                report['group_num'], report['user_fio']
            )
            continue
        # skip reports with unwanted status
        if (
            (report['status'] == '1' and 'awaiting' not in status_filter)  # and report['status_name'] == 'ожидает проверки' # noqa
            or (report['status'] == '2' and 'accepted' not in status_filter)  # and report['status_name'] == 'принят' # noqa
            or (report['status'] == '3' and 'rejected' not in status_filter)  # and report['status_name'] == 'не принят' # noqa
        ):
            logger.debug(
                "Report with status '%s' from '%s' (%s) is ignored by filter",
                report['status_name'], report['user_fio'], report['group_num']
            )
            continue
        # build full path and create missing subdirs if any
        path = os.path.join(
            output_dir, task['subject_name'][0],
            report['group_num'],
            task_name
        )
        ...
        new_filename = os.path.join(
            path,
            f"{report['user_fio']} [{report['status_name']}]"
            f"{original_extension}"
        )
        if keep_old:
            filename_suffix_id = 1
            while os.path.isfile(new_filename):
                new_filename_upd = os.path.join(
                    path,
                    f"{report['user_fio']} [{report['status_name']}]"
                    f"({filename_suffix_id}){original_extension}"
                )
                logger.warning(
                    "File '%s' already exists. "
                    "Will attempt to save file as '%s'",
        ...
        if not dry_run:
            with open(new_filename, 'wb') as f:
                f.write(res.content)
        print(
            f"{report['user_fio']} ({report['group_num']}) "
            f"[{report['status_name']}]: {report_url} -> {new_filename}"
        )
        if log_file:
            log_file.write(f"{report['filelink']}\t{new_filename}\n")
        saved_reports_count += 1
    logger.info(
        "Downloaded %s reports for %s: %s",
        saved_reports_count, task['subject_name'][0], task_name
    )
...
def main():
    ...
    if params.dry_run:
        logger.info(
            "Dry-run mode. No files will be saved to disk, "
            "although reports will still be downloaded"
        )
    ...
    try:
        ...
        logger.warning(
            "Unable to open downloads log file '%s'. "
            "No information about predownloaded reports is found",
            params.log_file
        )
        logger.debug(e)
    logger.info(
        "Total %s predownloaded reports were found", len(downloaded_reports)
    )
    # authenticate
    sess = requests_retry_session()
    if params.username:
        password = getpass.getpass(
            prompt=f"Password [{params.username}@pro.guap.ru]: "
        )
        # get authorization form;
        # without a get request the next post request fails even
        ...
        logger.error(
            ...
            "Please, specify a user name or a PHPSESSID cookie value "
            "for pro.guap.ru as a command line parameter for this script"
        )
        exit(1)
    # impersonate example URL:
    # https://pro.guap.ru/goswitch?_want_to_be_this_user=Polyak_MD
    if params.impersonate:
        logger.info("Trying to impersonate user '%s'", params.impersonate)
        res = sess.get(
            f'https://pro.guap.ru/goswitch?'
            f'_want_to_be_this_user={params.impersonate}',
            headers=headers
        )
        if res.status_code != 200:
            logger.error(
                "Unable to impersonate user '%s'. Server response: %s (%d)",
                params.impersonate, res.reason, res.status_code
            )
    ...
    if res.status_code != 200:
        logger.error("Unable to load page 'https://pro.guap.ru/inside#tasks'")
        exit(1)
    # extract user_id
    try:
    ...
    res = sess.post(
        'https://pro.guap.ru/gettasksdictionaries/', json={"iduser": user_id}
    )
    semester_ids = []
    semesters = {}
    ...
    if params.semester in semesters:
        ...
        if params.semester:
            logger.error(
                "Semester '%s' not found. Possible values are %s.",
                params.semester, semester_ids
            )
        print()
        print("\n".join([
            f"{sem['id']}: {sem['name']}"
        ...
        semester_id = get_user_choice(
            "Укажите идентификатор семестра: ", semester_ids
        )
    logger.info("Selected semester is '%s'", semesters[semester_id])
    # get tasks
    res = sess.post(
        'https://pro.guap.ru/gettasks/',
        json={"iduser": user_id, "semester": semester_id}
    )
    ...
    for i in res.json()['dictionaries']['groups']:
        for g in i['groups']:
            if g['id'] not in groups_by_task:
                ...
                logger.error(
                    "Duplicate group id found! $s | $s | $s",
                    i, g, groups_by_task[g['id']]
                )
    # parse tasks
    tasks = {}
    task_choices = []
    for task in res.json()['tasks']:
        ...
        # don't bother with different subject names for different groups,
        # just use the first one with index 0
        subject = task['subject_name'][0]
        groups = ", ".join([
            groups_by_task[str(group_id)]
            for idx, group_id in enumerate(task['groups'])
        ])
    ...
    if params.batch:
        for task_id in tasks:
            if task['hasReports']:
                logger.info(
                    "Processing reports for task '%s: %s'",
                    tasks[task_id]['subject_name'][0],
                    tasks[task_id]['name']
                )
        ...
        print("\n".join([
            f"{task['id']}: {task['subject_hr']}: "
            f"{task['name']} ({task['groups_hr']}) "
            f"[Студентов: {task['count']}; принято: {task['doneWork']}, "
            f"ожидают: {task['await']}]"
            for task in tasks.values() if task['hasReports']
        ]))
        print()
        task_id = get_user_choice(
            "Укажите идентификатор задания: ", task_choices
        )
        logger.info(
            "Processing reports for task '%s: %s'",
            tasks[task_id]['subject_hr'], tasks[task_id]['name']
        )
        total_saved_reports += process_reports(
            sess, tasks[task_id], params.output_dir,
            status_filter=params.status_filter,
    ...
    # общие словари:
    # res = sess.post('https://pro.guap.ru/gettasksdictionaries/', json={"iduser":440}) # noqa
    # задания за конкретный семестр:
    # res = sess.post('https://pro.guap.ru/gettasks/', json={"iduser":440, "semester":16}) # noqa
    # конкретное задание:
    # res = sess.post('https://pro.guap.ru/gettask/61088', json={"task_id":61088}) # noqa
